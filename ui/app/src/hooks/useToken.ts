      ...
      .map((asset: IAsset) => {
        ...
        pendingTransfers.forEach((transfer) => {
          if (
            ...
            if (transfer.interchainTx?.toChain.network === Network.SIFCHAIN) {
              console.log("hello", asset.symbol, transfer);
            }
            if (array) array.push(transfer);
          }
        });

        if (pendingImports.length || pendingExports.length) {
          console.log(asset.symbol, { pendingImports, pendingExports });
        }
